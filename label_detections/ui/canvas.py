...
class Box:
    ...
    ``label`` is the detector **family** the model is trained on; ``label_id``
    is which exact library label this is. Two fields on purpose: collapsing
    them is what would put every new label SKU back on the retraining
    treadmill. ``regions`` holds the barcode and text sub-areas nested inside
    the label, which the exporter walks straight past.
    """
    ...
    def to_dict(self) -> dict:
        ...
        # Only written when set, so an unidentified detection stays visibly
        # unidentified rather than carrying an empty identity.
        if self.label_id:
            data["label_id"] = str(self.label_id)
        if self.regions:
            data["regions"] = [dict(r) for r in self.regions]
        return data
    ...
    def from_dict(d: dict) -> "Box":
        ...
        label_id = str(d.get("label_id", "") or "")
        regions = [dict(r) for r in (d.get("regions") or []) if isinstance(r, dict)]
        ...
        return Box(float(d["x"]), float(d["y"]), float(d["w"]), float(d["h"]),
                   int(d.get("class_id", 0)), d.get("label", "spec_plate"), "box",
                   [], label_id, regions)
...
class ImageCanvas(QWidget):
    ...
    # One colour per detector family, in the same slate/blue palette as the
    # rest of the UI. Keyed by name rather than class id so adding a family
    # cannot silently recolour an existing one.
    _FAMILY_COLORS = {
        "battery_side": QColor(96, 165, 250),    # blue -- the whole face
        "spec_plate": QColor(34, 197, 94),       # green
        "warning_label": QColor(251, 146, 60),   # amber
        "cert_mark": QColor(168, 85, 247),       # violet
        "trace_tag": QColor(45, 212, 191),       # teal
        "promo_label": QColor(244, 114, 182),    # pink
        "code_patch": QColor(250, 204, 21),      # yellow
    }

    def _class_color(self, b: Box, selected: bool = False) -> QColor:
        if selected:
            return QColor(250, 204, 21)
        colour = self._FAMILY_COLORS.get(str(b.label or "").lower())
        if colour is not None:
            # A box the model found but nothing could name is drawn muted, so an
            # unidentified label reads differently at a glance from a named one.
            return colour if b.label_id else colour.darker(150)
        ...
        if self.pixmap:
            ...
            if self.show_annotations:
                for i, b in enumerate(self.boxes):
                    ...
                    if not low_detail:
                        # The identity when it is known, the family when it is
