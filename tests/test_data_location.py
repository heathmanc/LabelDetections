...
_STORAGE = Path(__file__).resolve().parents[1] / "label_detections" / "core" / "storage.py"
_WANT = {
...
class _Env:
    ...
    def __enter__(self):
        self._saved = {k: os.environ.get(k) for k in ("LOCALAPPDATA", "XDG_CONFIG_HOME", "LABELVISION_DATA_DIR")}
        os.environ["LOCALAPPDATA"] = self.localappdata
        os.environ.pop("XDG_CONFIG_HOME", None)
        if self.data_dir_env is None:
            os.environ.pop("LABELVISION_DATA_DIR", None)
        else:
            os.environ["LABELVISION_DATA_DIR"] = self.data_dir_env
        return self
