...
if not exist "dist\LabelVisionStudio\LabelVisionStudio.exe" (
    echo ERROR: dist\LabelVisionStudio\LabelVisionStudio.exe not found.
...
if not defined APPVER (
    echo ERROR: could not read APP_VERSION from label_detections\version.py
    exit /b 1
)
...
"%ISCC%" "installer\LabelVisionStudio.iss" /DAppVersion=%APPVER% /DEdition=%EDITION% || goto :error

echo.
echo === INSTALLER READY ===
dir /b "installer\LabelVisionStudio-%APPVER%-%EDITION%-setup.exe"
echo     installer\LabelVisionStudio-%APPVER%-%EDITION%-setup.exe
goto :eof
